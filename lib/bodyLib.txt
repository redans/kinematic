...
function fillTextareas() {
    ...
    normalizedSurfaceNormals = normr(surfaceNormals);
    ...
    //textareaInitialStiffness.value = initialStiffness;
    textareaInitialStiffness.value = initialStiffness.map(function (row) {
        return row[0].toExponential(3);
    }).join("\n");
...
function updateTextareaValues() {
    ...
    appliedForce = document.getElementById("applied-force-input").value.split(/\s*\n\s*/).map(row => row.split(/\s+/).map(parseFloat));
    appliedCoordinates = document.getElementById("applied-coordinates-input").value.split(/\s*\n\s*/).map(row => row.split(/\s+/).map(parseFloat));
    ...
    initialStiffness = document.getElementById("initial-stiffness-input").value.split("\n").map(row => row.split("\t").map(parseFloat));
 ...
 function ForceB(Fc, coordFc) {
    ...
    function ForceBrow(Fc, coordFc) {
        ...
        for (let i = 0; i < Fc.length; i++) {
            ...
            console.log("Fc value ForceB: ",coordFc,Fc)
    ...
    function kynCalcRow(n, p, mu, scale, Fc, coordFc) {
    ...
    //Fc in kynCalc:", Fc)
        ...
        const BB = ForceBrow(Fc, coordFc) // calculate force vector b
...
function findNegativeForceMuMinRow(nn, pp, Fcc, coordFcc, resolution) {

    let mu = 0;
    while (true) {
        // Calculate the forces and deformations for the current mu value
        const [FinterfaceF, WCSdeformF] = kynCalcRow(nn, pp, mu, 0.1, Fcc, coordFcc);
...
function findNegativeForceMuPlusRow(nn, pp, Fcc, coordFcc, resolution) {
    //console.log(Fcc)
        //ifingNegForceMuPlus: ",Fcc)
        let mu = 0;
        while (true) {
...
{
    function findMinMaxPerRow(array) {
        const result = [];
        const matrix = [];
        ...
        for (let i = 0; i < array.length; i++) {
            ...
            result.push({ min, max, difference });
        }

        for (let i = 0; i < result.length; i++) {
            const { min, max, difference } = result[i];
            matrix.push([min, max, difference]);
        }
...
function perturbFc(nn, pp, Fcc, coordFcc, perturbationRange, xn, resolution) {
    ...
    for (let i = 0; i < Fcc[0].length; i++) {
      const perturbedFcc = Fcc.slice(); // Create a copy of Fcc array
      const columnValues = [];
  
      const perturbationStart = Fcc[0][i] - perturbationRange;
      const perturbationEnd = Fcc[0][i] + perturbationRange;
      const perturbationStep = (perturbationEnd - perturbationStart) / xn;
  
      for (let j = 0; j <= xn; j++) {
        const perturbationValue = perturbationStart + j * perturbationStep;
        perturbedFcc[0][i] = perturbationValue;
...
function calculateMumaxValuesForce(appliedForce, appliedCoordinates, surfaceNormals, surfacePoints, forceAccuracy, numSteps, forceScanRange) {
    //onst range = 0.5;

    const rangeFx = linspace(appliedForce[0][0] - forceScanRange / 2, appliedForce[0][0] + forceScanRange / 2, numSteps);
    const rangeFy = linspace(appliedForce[0][1] - forceScanRange / 2, appliedForce[0][1] + forceScanRange / 2, numSteps);
    const rangeFz = linspace(appliedForce[0][2] - forceScanRange / 2, appliedForce[0][2] + forceScanRange / 2, numSteps);

    const Fcx = appliedCoordinates[0][0];
    const Fcy = appliedCoordinates[0][1];
    const Fcz = appliedCoordinates[0][2];
    ...
    // Perturb Fx
    for (let i = 0; i < numSteps; i++) {
        const perturbedForce = [rangeFx[i], appliedForce[0][1], appliedForce[0][2]];
        const mumaxValue = findNegativeForceMuPlus(surfaceNormals, surfacePoints, perturbedForce, [Fcx, Fcy, Fcz], forceAccuracy);
        mumaxValuesX.push(mumaxValue);
    }

    // Perturb Fy
    for (let i = 0; i < numSteps; i++) {
        const perturbedForce = [appliedForce[0][0], rangeFy[i], appliedForce[0][2]];
        const mumaxValue = findNegativeForceMuPlus(surfaceNormals, surfacePoints, perturbedForce, [Fcx, Fcy, Fcz], forceAccuracy);
        mumaxValuesY.push(mumaxValue);
    }

    // Perturb Fz
    for (let i = 0; i < numSteps; i++) {
        const perturbedForce = [appliedForce[0][0], appliedForce[0][1], rangeFz[i]];
        const mumaxValue = findNegativeForceMuPlus(surfaceNormals, surfacePoints, perturbedForce, [Fcx, Fcy, Fcz], forceAccuracy);
        mumaxValuesZ.push(mumaxValue);
    }

    const mumaxAll = [mumaxValuesX, mumaxValuesY, mumaxValuesZ];
    return mumaxAll;
}


function calculateMumaxValuesCoord(appliedForce, appliedCoordinates, surfaceNormals, surfacePoints, forceAccuracy, forceNumPoints, forceScanRange) {
    const range = forceScanRange / 2;
  
    const rangeCx = linspace(appliedCoordinates[0][0] - range, appliedCoordinates[0][0] + range, forceNumPoints);
    const rangeCy = linspace(appliedCoordinates[0][1] - range, appliedCoordinates[0][1] + range, forceNumPoints);
    const rangeCz = linspace(appliedCoordinates[0][2] - range, appliedCoordinates[0][2] + range, forceNumPoints);
  
    const Fx = appliedForce[0][0];
    const Fy = appliedForce[0][1];
    const Fz = appliedForce[0][2];
  
    const mumaxValuesX = [];
    const mumaxValuesY = [];
    const mumaxValuesZ = [];
  
    for (let i = 0; i < forceNumPoints; i++) {
      const perturbedCoordinates = [rangeCx[i], appliedCoordinates[0][1], appliedCoordinates[0][2]];
      const mumaxValue = findNegativeForceMuPlus(surfaceNormals, surfacePoints, [Fx, Fy, Fz], perturbedCoordinates, forceAccuracy);
      mumaxValuesX.push(mumaxValue);
    }
  
    for (let i = 0; i < forceNumPoints; i++) {
      const perturbedCoordinates = [appliedCoordinates[0][0], rangeCy[i], appliedCoordinates[0][2]];
      const mumaxValue = findNegativeForceMuPlus(surfaceNormals, surfacePoints, [Fx, Fy, Fz], perturbedCoordinates, forceAccuracy);
      mumaxValuesY.push(mumaxValue);
    }
  
    for (let i = 0; i < forceNumPoints; i++) {
      const perturbedCoordinates = [appliedCoordinates[0][0], appliedCoordinates[0][1], rangeCz[i]];
      const mumaxValue = findNegativeForceMuPlus(surfaceNormals, surfacePoints, [Fx, Fy, Fz], perturbedCoordinates, forceAccuracy);
      mumaxValuesZ.push(mumaxValue);
    }
...
function calcNewSetpoint(start_value, new_value, new_setpoint) {
    ...
    const CoordX = start_value[0][0] + new_value[new_setpoint[0]];
    const CoordY = start_value[0][1] + new_value[new_setpoint[1]];
    const CoordZ = start_value[0][2] + new_value[new_setpoint[2]];

    const coordNew = [CoordX, CoordY, CoordZ];
console.log("newsetpoint : ",coordNew)
    const formattedCoordNew = coordNew.map((element) => {
        if (typeof element !== 'number') {
            throw new Error('Invalid coordinate value: ' + element);
        }
...
function dotMult(array1, array2) {
    if (!array1 || !array2) {
        throw new Error("Input arrays are undefined or null.");
    }
    ...
    for (let i = 0; i < numRows; i++) {
        ...
        for (let j = 0; j < numCols; j++) {
            const product = array1[i][j] * array2[i][j];
            row.push(product);
        }
        result.push(row);
    }
...
function replicateArray(array, numRows, numCols) {
    const replicatedArray = Array.from({ length: numRows }, () => Array(numCols).fill());

    for (let i = 0; i < numRows; i++) {
        for (let j = 0; j < numCols; j++) {
            replicatedArray[i][j] = array[j];
        }
    }
